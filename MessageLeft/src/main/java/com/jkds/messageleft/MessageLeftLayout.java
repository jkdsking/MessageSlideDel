 ...
 * Date: 16/04/24
 */
public class MessageLeftLayout extends ViewGroup {
    ...
    //存储的是当前正在展开的View
    private static MessageLeftLayout mViewCache;
    ...
    public MessageLeftLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MessageLeftLayout(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @param ios
     */
    public MessageLeftLayout setIos(boolean ios) {
        isIos = ios;
        return this;
    }
     ...
     * @return
     */
    public MessageLeftLayout setLeftSwipe(boolean leftSwipe) {
     ...
     * @return
     */
    public static MessageLeftLayout getViewCache() {
        return mViewCache;
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true;
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.MessageLeftLayout, defStyleAttr, 0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = ta.getIndex(i);
            //如果引用成AndroidLib 资源都不是常量，无法使用switch case
            if (attr == R.styleable.MessageLeftLayout_swipeEnable) {
                isSwipeEnable = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.MessageLeftLayout_ios) {
                isIos = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.MessageLeftLayout_leftSwipe) {
...
/*                    if (!mScroller.isFinished()) {
    ...
    public void smoothExpand() {
        ...
        invalidate();*/
        //展开就加入ViewCache：
        mViewCache = MessageLeftLayout.this;
